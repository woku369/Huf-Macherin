...
ipcMain.handle('termine:addMultiple', async (_event, terminDaten) => {
    ...
    for (const pferdId of terminDaten.pferdIds) {
        const info = stmt.run(pferdId, terminDaten.datum, 0, // rechnung = false per default
        terminDaten.bemerkung, terminDaten.ende || null, terminDaten.status || 'vorreserviert', null);
